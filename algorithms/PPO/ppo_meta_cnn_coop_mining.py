"""
Meta-PPO (feed-forward PPO) for coop_mining with sampled teammate policies.
Same trial/episode training flow as RL2, but no recurrent state transfer.
"""
...
def make_policy_evaluator(config: dict):
    ...
    def _eval_once(policy_params, rng, controlled_agent, teammate_policy_ids):
        ...
        def _step(carry, _):
            ...
            logits, _ = network.apply(policy_params, obs_ctrl)
            ...
            # Eval is always the final episode reward.
...
def make_train_stepper(config):
    ...
    ppo_loss_scan_unroll = max(1, int(config.get("PPO_LOSS_SCAN_UNROLL", 4)))

    if num_envs % num_minibatches != 0:
        raise ValueError("NUM_ENVS must be divisible by NUM_MINIBATCHES for minibatching.")
    ...
    def _loss_fn(params, traj_mb, adv_mb, target_mb):
        def _ff_forward(_, inps):
            obs_t, action_t = inps
            logits_t, value_t = network.apply(params, obs_t)
    ...
    def _update_step(runner_state, _):
        ...
        def _env_step(carry, _):
            ...
            obs_ctrl = gather_by_agent(last_obs, controlled_agent)
            logits, value = network.apply(train_state.params, obs_ctrl)
        ...
        _, last_value = network.apply(train_state.params, last_obs_ctrl)
        ...
        def _update_epoch(carry, _):
            ...
            def _update_minibatch(ts, minibatch):
                ...
                (_, metrics), grads = grad_fn(ts.params, traj_mb, adv_mb, target_mb)
...
def single_run(config):
    ...
    wandb.init(
        ...
        name="ppo_meta_cnn_coop_mining",
    )
    train_state = train_and_evaluate(config)

    print("** Saving Results **")
    filename = f"{config['ENV_NAME']}_ppo_meta_seed{config['SEED']}"
    save_path = f"./checkpoints/ppo_meta/{filename}.pkl"
...
def tune(default_config):
    """
    WandB sweep entry for Meta-PPO.
    ...
    sweep_config = {
        "name": "ppo_meta_coop_mining_lr_sweep",
        "method": "grid",
        "metric": {
    ...
    def wrapped_make_train():
        ...
        run_name = f"ppo_meta_lr{config['LR']}_seed{config['SEED']}"
...
@hydra.main(version_base=None, config_path="config", config_name="ppo_meta_cnn_coop_mining")
def main(config):
