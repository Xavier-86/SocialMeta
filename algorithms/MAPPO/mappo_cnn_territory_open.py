...
def single_run(config):
    ...
    wandb.init(
        ...
        name=f'mappo_cnn_territory_open'
    )
...
def evaluate(params, env, save_path, config):
    ...
    pics.append(img)
    root_dir = f"evaluation/territory_open"
...
@hydra.main(version_base=None, config_path="config", config_name="mappo_cnn_territory_open")
def main(config):
    if config["TUNE"]:
        tune(config)
    else:
