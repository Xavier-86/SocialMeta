...
from socialmeta import make
from socialmeta.environments.coop_mining.coop_mining import Actions, ViewConfig
from socialmeta.environments.coop_mining.coop_mining import Items
...
def greedy_policy(obs, state, env):
    ...
    For each agent:
      ...
      4) With 10% probability or if no target found, take a random action instead.
    """
    global agent_targets
    actions = []
    ...
    for i in range(env.num_agents):
        # With 10% probability, take a random action
...
def main():
    ...
    for n_a in agent_pop_sizes:
        ...
        env = make(
            ...
            jit=False,
            view_config=ViewConfig(forward=6, backward=1, left=4, right=4),
        )
        ...
        for o_t in range(num_outer_steps):
            ...
            fixed_agent_positions = jnp.array([
                # [17, 3, 0],
                # [18, 4, 1],
                # [18, 2, 3],
                # [19, 3, 2],
            ...
            # Override the random spawns
            old_state = old_state.replace(agent_locs=fixed_agent_positions)
